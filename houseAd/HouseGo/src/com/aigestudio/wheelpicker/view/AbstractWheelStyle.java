...
/**
 * 抽象滚轮样式 Abstract of WheelView's style
 *
...
abstract class AbstractWheelStyle {
	...
	void computeTextSize() {
		...
		} else if (!TextUtils.isEmpty(view.textWidthMaximum) && !TextUtils.isEmpty(view.textHeightMaximum)) {
			paint.getTextBounds(view.textWidthMaximum, 0, view.textWidthMaximum.length(), textBounds);
			maxTextWidth = textBounds.width();
			paint.getTextBounds(view.textHeightMaximum, 0, view.textHeightMaximum.length(), textBounds);
			maxTextHeight = textBounds.height();
	...
	void onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			if (direction.isValidArea(event, view.getWidth(), view.getHeight()))
				break;
	...
	void computeScroll() {
		...
		if ((unitTotalMove == finalUnit || finalUnit == -1) && unitTotalMove % unit == 0) {
			int tmpIndex = view.itemIndex - unitTotalMove / unit;
			if (tmpIndex < 0)
				tmpIndex = 0;
			if (tmpIndex >= view.data.size())
				tmpIndex = view.data.size() - 1;
	...
	private void correctDegree() {
		...
		if (degreeRemainder != 0) {
			if (degreeRemainder >= unit / 2.0F)
				...
				correctScroll(degreeRemainder, -degreeRemainder);
			view.invalidate();
		}
	}
	...
	private void correctScroll(int endBack, int endForward) {
		if (unitTotalMove < 0)
			direction.startScroll(scroller, unitTotalMove, endBack, 300);
		else
			direction.startScroll(scroller, unitTotalMove, endForward, 300);
