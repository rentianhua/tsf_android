...
/**
 * 滚轮对外提供实际功能的类 Function interface implement of WheelView
 *
 * @author AigeStudio 2015-12-03
 * @author AigeStudio 2015-12-08 增加接口方法 移除枚举类型改由int替代常量定义
 * @version 1.0.0 beta
 */
public class WheelPicker extends WheelView implements IWheelPicker {
	/**
	 * 滚轮方向枚举类 目前为止WheelPicker仅仅支持垂直方向的滚轮滚动 水平方向的滚轮滚动将在以后的版本中支持 Enum of
	 * WheelView's direction So far, WheelPicker only support vertical scrolling
	 * We will support horizontal scrolling in a later version
	 */
	static final int HORIZONTAL = 0, VERTICAL = 1;

	/**
	 * 滚轮样式枚举类 目前为止WheelPicker仅仅支持直向和弯曲两种样式 Enum of WheelView's direction So
	 * far, WheelPicker only support straight and curved style
	 */
	public static final int STRAIGHT = 0, CURVED = 1;

	/**
	 * WheelView滚动状态标识值 SCROLL_STATE_IDLE标识WheelView处于静止时的状态
	 * SCROLL_STATE_DRAGGING标识WheelView处于拖拽时的状态
	 * SCROLL_STATE_SCROLLING标识WheelView处于滚动时的状态 Flag of scroll state
	 * SCROLL_STATE_IDLE Set when WheelView is idle SCROLL_STATE_DRAGGING Set
	 * when WheelView is dragging SCROLL_STATE_SCROLLING Set when WheelView is
	 * scrolling
	 */
