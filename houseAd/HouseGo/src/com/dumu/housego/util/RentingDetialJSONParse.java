    ...
    public static RentingDetail parseSearch(String json) throws JSONException {
        ...
        if (j.has("tongqu") && !j.isNull("tongqu")) {
            ...
            if (tongqu != null && tongqu.length() > 0) {
                n.tongqu = new ArrayList<RentingDetail>();
                for (int i = 0; i < tongqu.length(); i++) {
                    JSONObject tq = tongqu.getJSONObject(i);
                    RentingDetail item = new RentingDetail();
                    ...
                    item.setZongceng(tq.getString("zongceng"));
                    ...
                    item.setZujin(tq.getString("zujin"));

                    item.setMianji(tq.getString("mianji"));
