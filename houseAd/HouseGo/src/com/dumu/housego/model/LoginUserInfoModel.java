                    ...
                    public void onResponse(String response) {
                        try {
                            if (response != null) {
                                ...
                                try {
                                    if (obj.has("info") && obj.get("info") != null && obj.getString("info") != null && !obj.getString("info").equals("null")) {
                                        JSONObject j = obj.getJSONObject("info");
                                        userinfo.setRealname(j
                                                .getString("realname"));

                                        userinfo.setJiav(j.getString("jiav"));

                                        if (obj.getString("modelid").equals("36")) {
                                            userinfo.setWorktime(j
                                                    .getString("worktime"));
                                            userinfo.setDengji(j
                                                    .getString("dengji"));
                                            userinfo.setCardnumber(j
                                                    .getString("cardnumber"));
                                            userinfo.setMainarea(j
                                                    .getString("mainarea"));
                                            userinfo.setLeixing(j
                                                    .getString("leixing"));
                                            userinfo.setBiaoqian(j
                                                    .getString("biaoqian"));
                                            userinfo.setSfzpic(j
                                                    .getString("sfzpic"));
                                            userinfo.setConame(j
                                                    .getString("coname"));
                                        }
                                    }
                                } catch (Exception e) {
